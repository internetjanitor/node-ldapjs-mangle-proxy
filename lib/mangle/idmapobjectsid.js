...
IdMapObjectSid.prototype.request = function (req) {
...
IdMapObjectSid.prototype.result = function (res) {
  var sid = new ObjectSid().fromBinary(res.output['objectSid']);
  var id = this.objectSidToId(sid);
  if (util.isObjectClass('user', res.output) && id) {
    res.output[this.params.uidField] = id;
  } else if (util.isObjectClass('group', res.output) && id) {
    res.output[this.params.gidField] = id;
  }
  res.next();
}
